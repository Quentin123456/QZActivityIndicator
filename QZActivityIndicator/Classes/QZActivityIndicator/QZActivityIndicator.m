...
@implementation QZActivityIndicator

#pragma mark 加载中动画
+ (instancetype)showHUDShowText:(NSString *)showText {
    
    QZActivityIndicator *hud = [[self alloc] initWithFrame:[UIScreen mainScreen].bounds showText:showText HUDType:QZActivityIndicatorLoadingType];
...
+ (instancetype)showSuccessfulAnimatedText:(NSString *)ShowText {
    ...
    [QZActivityIndicator hideAllHUDAnimated:NO];
    QZActivityIndicator *hud = [[self alloc] initWithFrame:[UIScreen mainScreen].bounds showText:ShowText HUDType:QZActivityIndicatorSuccessfulAnimatedType];
...
+ (instancetype)showErrorAnimatedText:(NSString *)ShowText {
    ...
    [QZActivityIndicator hideAllHUDAnimated:NO];
    QZActivityIndicator *hud = [[self alloc] initWithFrame:[UIScreen mainScreen].bounds showText:ShowText HUDType:QZActivityIndicatorErrorAnimatedType];
    [hud show:YES view:hud.toast];
    [[UIApplication sharedApplication].keyWindow addSubview:hud];
    [self performSelector:@selector(DelayHideHud) withObject:nil afterDelay:1.5];
    return hud;
...
+ (instancetype)showDpromptText:(NSString *)showText {
    ...
    [QZActivityIndicator hideAllHUDAnimated:NO];
    QZActivityIndicator *hud = [[self alloc] initWithFrame:[UIScreen mainScreen].bounds showText:showText HUDType:QZActivityIndicatorpromptTextType];
...
+ (NSUInteger)hideAllHUDAnimated:(BOOL)animated
{
    ...
    {
        if ([aView isKindOfClass:[QZActivityIndicator class]])
        {
            [huds addObject:aView];
        }
    }
    
    for (QZActivityIndicator *hud in huds) {
        
        if (hud.type== QZActivityIndicatorpromptTextType) {
...
- (instancetype)initWithFrame:(CGRect)frame showText:(NSString *)showText HUDType:(QZActivityIndicatorType)type {
    ...
    if (self)
    {
        ...
        CGSize size = [QZPublicManager workOutSizeWithStr:showText andFont:14 value:[NSValue valueWithCGSize:CGSizeMake(self.frame.size.width-30, 999)]];
        if (size.width>100) {
        ...
        if (type==QZActivityIndicatorLoadingType) {
            //加载
            [self loadingAnimated];
        } else if (type==QZActivityIndicatorSuccessfulAnimatedType) {
            //成功
            [self pictureCorrect];
        } else if (type==QZActivityIndicatorErrorAnimatedType) {
            //失败效果
            [self picturError];
        } else if (type==QZActivityIndicatorpromptTextType) {
            //提示文字
            [self DpromptText];
        }
...
- (void)DpromptText {
    ...
    CGSize size = [QZPublicManager workOutSizeWithStr:_tipText andFont:14 value:[NSValue valueWithCGSize:CGSizeMake(self.frame.size.width-40, 999)]];
    if (size.width>100) {
...
- (void)setType:(QZActivityIndicatorType)type {
